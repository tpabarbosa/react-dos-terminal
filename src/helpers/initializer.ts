...
import { FakeAttributes, FakeFile, FakeFileSystem } from "../contexts/FileSystemContext";
...
const createFakeFileSystem = (internal?: FakeFile[], external?: FakeFile[] | undefined): FakeFileSystem => {
    ...
    let totalSize: number = Object.values(files).reduce((acc, file)=> acc + file.s, 0)
...
const createFileSystemFromArray = (files: FakeFile[]) => {
    let final:FakeFileSystem = {} as FakeFileSystem;
    let totalSize = 0;
    const obj = files.reduce((acc, file)=> {
        ...
        const newFileType = file.type === 'directory' ? 'd' : file.type === 'file' ? 'f' : file.type==='exec-file' ? 'e' : 's' as 'd' | 'f' | 'e' | 's';
...
const mergeEqualDirs = (a: FakeFile[], b: FakeFile[]) => {
    ...
    dirsA.forEach(dirA => {
        const dirB = b.filter(item => item.type === 'directory' && item.name === dirA.name);
        if (dirB[0]) {
...
const calcFileSize = (name: string, file: {c: string | string[] | FakeCommand | FakeFileSystem, a: FakeAttributes, t: 'd' | 'f' | 'e' | 's'}) => {
...
const renameEqualFiles = (contentA: FakeFile[], contentB: FakeFile[]) => {
    ...
    contentB.forEach(file => {
        if (file.type === 'file' || file.type === 'exec-file' || file.type === 'system-file') {
...
const mainAttrib = (a: FakeFile, b: FakeFile) => {
    ...
    else if (a.attributes[0]==='r' || b.attributes[0]==='r') {
        return 'r'
    }
    else {
        return 'w'
