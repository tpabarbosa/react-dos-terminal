...
const help = [
    ...
    `By now only a few commands are available, but much more can be done to improve it. See ${commandsHelper.link(
        'https://github.com/tpabarbosa',
        'documentation'
    )} to get more information.`,
    '',
    '',
    `Thanks to "VileR" from ${commandsHelper.link(
        'https://int10h.org/oldschool-pc-fonts',
        'THE OLDSCHOOL PC FONT RESOURCE'
    )} for adapting and providing various oldschool fonts. WebPlus_IBM_VGA_9x16.woff was the chosen font for this project.`,
    '',
]

const run = (): Command => {
    return {
        output: [{ action: 'add', value: help }],
    }
}
