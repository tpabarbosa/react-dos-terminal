...
const run = ({args, files, actualDir}: CommandProps): Command => {
    ...
    const output = (files: FakeFileSystem, finalPath: string, content: FakeFileSystem, showHidden: boolean) => {
        ...
        if ( !content.files) {
            return [...getInitialOutput(finalPath), ...getFinalOutput(files,filesNumber, dirsNumber, size)];
        }
    
        const dirContent = Object.keys(content.files).reduce((acc, item) => {
            let text = '';
            if (showHidden || content.files[item].a[1]!=='h') {
                if (content.files[item].t === "f" || content.files[item].t === "e" || content.files[item].t === "s") {
                    filesNumber++;
                    size += content.files[item].s;
                    text = `${item}${space.repeat(28-item.length)}${content.files[item].s.toLocaleString('en-US', { minimumFractionDigits: 0 })}`;
        ...
        return [...getInitialOutput(finalPath), ...dirContent, ...getFinalOutput(files, filesNumber, dirsNumber, size)]
    ...
    const getFinalOutput = (files: FakeFileSystem, filesNumber: number, dirsNumber: number, size:number) => {
        const freeSize = (4194304 - files.totalSize).toLocaleString('en-US', { minimumFractionDigits: 0 });
        
        return [
            '',
            `   ${filesNumber} File(s)   ${size.toLocaleString('en-US', { minimumFractionDigits: 0 }) ?? 0} bytes`,
    ...
    const device = () => {
        const device =  navigator.userAgent.match(/(MSIE|(?!Gecko.+)Firefox|(?!AppleWebKit.+Chrome.+)Safari|(?!AppleWebKit.+)Chrome|AppleWebKit(?!.+Chrome|.+Safari)|Gecko(?!.+Firefox))(?: |\/)([\d.apre]+)/);
        return device ?? 'OS';
    }
    ...
    if (args === '' || (match && args === match[0])) { 
        ...
        if (content) {
            return {
                output: [
                    {action: 'add', value: output(files, actualDir, content, showHidden)}
                ]
            };
        }
    }
    ...
    return { output: [{action: 'add', value: ['Error: directory does not exist.', '']}]}
}
